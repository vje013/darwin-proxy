...
otherwise. Optional deps (PIL, numpy) are imported inside the guarded tests so the
core suite collects cleanly when the images extra is absent."""
import pytest
...
def test_unavailable_raises_clearly(monkeypatch):
    # runs everywhere, including hosts without the images extra: no optional imports here
...
def test_email_redacted_from_image():
    ...
    assert "alice@acme.com" in pytesseract.image_to_string(img)
    out = _blank_redactor().redact_image(img)
    assert "alice@acme.com" not in pytesseract.image_to_string(out)
    assert (np.asarray(img) != np.asarray(out)).any()
...
def test_no_pii_leaves_image_untouched():
    ...
    out = _blank_redactor().redact_image(img)
    assert int((np.asarray(img) != np.asarray(out)).any(axis=2).sum()) == 0
