...
def test_gender_preserved():
    p = Proxy()
    out_m, _, _ = p.abstract_record({"First Name": "John"})
    out_f, _, _ = p.abstract_record({"First Name": "Lisa"})
...
def test_state_stays_in_region():
    ...
    p = Proxy()
    out, _, _ = p.abstract_record({"State": "Connecticut"})
...
def test_consistency_same_input_same_output():
    p = Proxy()
    a, _, _ = p.abstract_record({"First Name": "John", "Last Name": "Reed"})
    b, _, _ = p.abstract_record({"First Name": "John", "Last Name": "Reed"})
...
def test_email_derived_from_name():
    p = Proxy()
    out, _, _ = p.abstract_record({"First Name": "John", "Last Name": "Reed", "Email": "john.reed@gmail.com"})
